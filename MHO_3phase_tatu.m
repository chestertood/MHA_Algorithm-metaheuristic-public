...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    for u = 1:Num_iter
        ...
        for i = 1:nPop            
            ...
            for j = 1:nVar
                ...
                dim = minVals(j) + (rangeVals(j)) * rand(); % สุ่มค่าในขอบเขต
    ...
    for u = Num_iter+1:Num_iter*2
        ...
        points = [pop(index_sort(1:4)).Position];
        points = reshape(points,nVar,4).';
        ...
        for i = matches_roaming            
            ...
            Test_delta1 = Delta1;
            attack = Delta1*((0.9-0.5)*rand()+0.5);
            for j = 1:nVar
                new_pos_plus = Delta1(j) + attack(j);
                Test_delta1(j) = new_pos_plus;
                cost1 = CostFunction(Test_delta1);
                new_pos_minus = Delta1(j) - attack(j);
                Test_delta1(j) = new_pos_minus;
                cost2 = CostFunction(Test_delta1);
                if cost1<cost2
                    Test_delta1(j) = new_pos_plus;
                end                  
...
%             Delta2 = Delta1+ run2*(GlobalBest.Position -Delta1);
       ...
       targetrole = 'support';      
       matches_support = find(cellfun(@(b) isequal(b, targetrole), {pop.role}));    
      
       run3 = Max_run - range_run*(u-(Num_iter+1))/((Num_iter*2)-(Num_iter+1));
       run4 = Min_run + range_run*(u-(Num_iter+1))/((Num_iter*2)-(Num_iter+1));
...
%        for i = matches_range
%            pos = pop(i).Position;
%            delta = GlobalBest.Position - pos;
%            run5 = Min_run + range_run*(u-(Num_iter+1))/((Num_iter*2)-(Num_iter+1));
%            new_pos = pos+delta*0.9;
%            for j = 1:nVar
%                 dim = new_pos(j);                             
%                 new_dim_plus = dim + run5*dim;
%                 new_dim_minus = dim - run5*dim;
...
%        end
         ...
         targetrole = 'range';      
         matches_range = find(cellfun(@(b) isequal(b, targetrole), {pop.role}));
         run5 = Max_run - range_run*(u-(Num_iter+1))/((Num_iter*2)-(Num_iter+1));
         run6 = Min_run + range_run*(u-(Num_iter+1))/((Num_iter*2)-(Num_iter+1));
         safety_factor = rand();
         danger_signal = rand();%มาเปลี่ยนชื่อทีหลัง
         Gbest = GlobalBest.Position;
         for i = matches_range
             pos = pop(i).Position;
             new_pos = pos +  0.5*(GlobalBest.Position - pos);
             %new_pos = GlobalBest.Position + r * (pos - GlobalBest.Position) / norm(pos - GlobalBest.Position);
             if safety_factor > 0.7 %บอสติดสตั้น เลยเข้ามาตีใกล้ๆ
                 for j = 1:nVar
                     dim = new_pos(j);
                     new_pos_dim_plus = dim + 0.05 * (pos(j) - Gbest(j)) / norm(pos(j) - Gbest(j));
                     new_pos_dim_minus = dim - 0.05 * (pos(j) - Gbest(j)) / norm(pos(j) - Gbest(j));
                     cost_plus_r = CostFunction(new_pos_dim_plus);
                     cost_minus_r =CostFunction(new_pos_dim_minus);
                     if cost_plus_r < cost_minus_r
                         new_pos(j) = new_pos_dim_plus;
                     else
                         new_pos(j) = new_pos_dim_minus;
                     end

                 end
                 
                 if danger_signal > 0.7 %บอสโจมตีมา
...
%                  if danger_signal > 0.7 %บอสโจมตีมา
                     for j = 1:nVar
                        dim = new_pos(j);
                        new_pos_dim_plus = dim + run5 * (pos(j) - Gbest(j)) / norm(pos(j) - Gbest(j));
                        new_pos_dim_minus = dim - run5 * (pos(j) - Gbest(j)) / norm(pos(j) - Gbest(j));
                        cost_plus_r = CostFunction(new_pos_dim_plus);
                        cost_minus_r =CostFunction(new_pos_dim_minus);
                        if cost_plus_r < cost_minus_r
                            new_pos(j) = new_pos_dim_plus;
        ...
        for i = 1:nPop
           ...
           for j = 1:nVar
                ...
                if (dim<1 && dim>=0) || (dim > -1 && dim <= 0)
                    ...
                    order_of_magnitude = floor(log10(abs(dim)));  % Handles the scale
                    
                    % Step 2: Shift decimal point to make the first significant digit an integer
                    scaled_value = dim * 10^(-order_of_magnitude);
                    
                    % Step 3: Truncate or round
                    rounded_scaled_value = floor(scaled_value);  % Truncate (use `round` for rounding)
                    
                    % Step 4: Scale back to the original magnitude
                    sperate_number = rounded_scaled_value * 10^(order_of_magnitude);
