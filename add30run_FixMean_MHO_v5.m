...
Problems = arrayfun(@(x) ['F', num2str(x)], 1:23, 'UniformOutput', false); % 1:23 to 1:30
...
% Load Benchmark Function
    ...
    num_runs = 30; %เพิ่มในส่วนของการรัน num_runs รอบ :D โดยสามารถทดสอบได้ เช่น last_BestCost_GA() = มันจะโชว์ num_runs ค่า 
    % สมมติกด step รันครั้งเดียวได้ 3119.144 | mean_GA() ก็จะคิด 3119.144+0+../num_runs = 103.9714
    last_BestCost_GA = zeros(num_runs, 1);
    last_BestCost_PSO = zeros(num_runs, 1);
    last_BestCost_GWO = zeros(num_runs, 1);
    last_BestCost_ABC = zeros(num_runs, 1);
    last_BestCost_SA = zeros(num_runs, 1);
    last_BestCost_WO = zeros(num_runs, 1);
    Last_BestCost_PUMA = zeros(num_runs, 1);
    last_BestCost_HBO = zeros(num_runs, 1);
    last_BestCost_WOA = zeros(num_runs, 1);

    for run = 1:num_runs %add
        % Run MHO Algorithm
        ...
        last_BestCost_MHO(run) = Convergence(end); %change
        mean_MHO = mean(last_BestCost_MHO);
        ...
        max_GA = max(Conv_GA);
        min_GA = min(Conv_GA);
        last_BestCost_GA(run) = Conv_GA(end);
        mean_GA = mean(last_BestCost_GA);
        ...
        last_BestCost_PSO(run) = Conv_PSO(end);
        mean_PSO = mean(last_BestCost_PSO);
        ...
        last_BestCost_GWO(run) = Conv_GWO(end);
        mean_GWO = mean(last_BestCost_GWO);
        ...
        last_BestCost_ABC(run) = Conv_ABC(end);
        mean_ABC = mean(last_BestCost_ABC);
        ...
        max_SA = max(Conv_SA);
        min_SA = min(Conv_SA);
        last_BestCost_SA(run) = Conv_SA(end);
        mean_SA = mean(last_BestCost_SA);
        ...
        last_BestCost_WO(run) = Convergence_curve_WO(end);
        mean_WO = mean(last_BestCost_WO);
        ...
        last_BestCost_PUMA(run) = Convergence_PUMA(end);
        mean_PUMA = mean(last_BestCost_PUMA);
        ...
        last_BestCost_HBO(run) = Convergence_curve_HBO(end);
        mean_HBO = mean(last_BestCost_HBO);
        ...
        last_BestCost_WOA(run) = Convergence_curve_WOA(end);
        mean_WOA = mean(last_BestCost_WOA);
...
%% Results
% Save all results
save('optimization_results.mat', 'results'); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    สิ่งที่เพิ่มถึงแค่นี้    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     positions = VarMin + sobolPoints .* (VarMax - VarMin);
    ...
    for u = 1:MaxIt 
        ...
        if u <=10            
            ...
            normalizedPoints = (points - minVals) ./ rangeVals;
            for i = 1:nPop            
            ...
            if u == 10
                ...
                % สร้างข้อมูล 30 มิติจากตำแหน่งทั้งหมด
                data = reshape([pop.Position], nVar, nPop).'; % nPop x nVar (30 dim)
        ...
        if  (u>10) && (u<=20)
            ...
            for i = 1:3
                ...
                points = [points,pop(i).Position];
            end
            
            mean_pos = (total_pos/length(matches_roaming)); 
           ...
           run3 = Min_run + range_run*(u-11)/(MaxIt-11);
...
function showfig = plotG(data,pop)

    % ลดมิติด้วย PCA
    [coeff, score, ~] = pca(data);  % score คือตำแหน่งที่ลดมิติแล้ว
    
    % เก็บตำแหน่งใน 2D
    positions_2D = score(:, 1:2);  % เลือก 2 แกนแรกที่อธิบายข้อมูลได้มากที่สุด
    
    % อัปเดตตำแหน่งใน pop
    for i = 1:nPop
        pop(i).Position_2D = positions_2D(i, :); % เพิ่มฟิลด์ใหม่สำหรับตำแหน่ง 2D
    end
    % แยกข้อมูลสำหรับการพล็อต
    x = positions_2D(:, 1); % แกน x
    y = positions_2D(:, 2); % แกน y
    
    % พล็อตตำแหน่ง 2 มิติของประชากร
    
    showfig = figure;
    scatter(x, y, 50, 'filled');
    title('Population in 2D Space');
    xlabel('PCA Dim 1');
    ylabel('PCA Dim 2');
    grid on;
