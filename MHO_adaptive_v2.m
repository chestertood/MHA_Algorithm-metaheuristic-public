...
F = 'F11'; % Change this to any function ID from 'F1' to 'F23'
...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    ratio = 1/Number_hunter_each_party;
    ...
    if Number_hunter_each_party == 2
        for u = 1:MaxIt
            for i = 1:nPop        
                total_range = abs(VarMax - VarMin);           
                roaming_bounds = [VarMin, VarMin + ratio * total_range];            
                frontline_bounds = [VarMin + ratio * total_range, VarMax];            
                            
                for j = 1:Number_hunter_each_party
                    role = mod(j - 1, Number_hunter_each_party); % Assign roles: 0=Roaming, 1=Frontline, 2=Support, 3=Range
                    if role == 0
    ...
    if Number_hunter_each_party == 3
        for u = 1:MaxIt
            for i = 1:nPop        
                total_range = abs(VarMax - VarMin);           
                roaming_bounds = [VarMin, VarMin + ratio * total_range];            
                frontline_bounds = [VarMin + ratio * total_range, VarMin + ratio*(2)* total_range];            
                support_bounds = [VarMin + ratio*(2) * total_range, VarMax];            
    ...
    if Number_hunter_each_party == 4
        ...
        roaming_bounds = [VarMin, VarMin + ratio * total_range];                
        frontline_bounds = [VarMin + ratio * total_range, VarMin + ratio*2 * total_range];               
        support_bounds = [VarMin + ratio*2 * total_range, VarMin + ratio*3 * total_range];                
        range_bounds = [VarMin + ratio*3 * total_range, VarMax]; 
        ...
        bestBound = pop(bestIndex).bound;
        second_bestbound = pop(secondBestIndex).bound;
        
        VarMin_Gbest = bestBound(1);
        VarMax_Gbest = bestBound(2);
        VarMin_Lbest = second_bestbound(1);
        VarMax_Lbest = second_bestbound(2);
        ...
        for u = 1:MaxIt
            for i = 1:nPop       
                % Gbest 
                total_range_Gbest = abs(VarMax_Gbest - VarMin_Gbest);                 
                roaming_bounds_Gbest = [VarMin_Gbest, VarMin_Gbest + ratio * total_range_Gbest];                
                frontline_bounds_Gbest = [VarMin_Gbest + ratio * total_range_Gbest, VarMin_Gbest + ratio*2 * total_range_Gbest];               
                support_bounds_Gbest = [VarMin_Gbest + ratio*2 * total_range_Gbest, VarMin_Gbest + ratio*3 * total_range_Gbest];                
                range_bounds_Gbest = [VarMin_Gbest + ratio*3 * total_range_Gbest, VarMax_Gbest]; 
                
                % LBest
                total_range_Lbest = abs(VarMax_Lbest - VarMin_Lbest);                
                roaming_bounds_Lbest = [VarMin_Lbest, VarMin_Lbest + ratio * total_range_Lbest];                
                frontline_bounds_Lbest = [VarMin_Lbest + ratio * total_range_Lbest, VarMin_Lbest + ratio*2 * total_range_Lbest];               
                support_bounds_Lbest = [VarMin_Lbest + ratio*2 * total_range_Lbest, VarMin_Lbest + ratio*3 * total_range_Lbest];                
                range_bounds_Lbest = [VarMin_Lbest + ratio*3 * total_range_Lbest, VarMax_Lbest]; 
                
                
                for j = 1:Number_hunter_each_party
                    role = mod(j - 1, Number_hunter_each_party); % Assign roles: 0=Roaming, 1=Frontline, 2=Support, 3=Range
                    if role == 0
                        bounds_Gbest = roaming_bounds_Gbest;
                        bounds_Lbest = roaming_bounds_Lbest;
                    elseif role == 1
                        bounds_Gbest = frontline_bounds_Gbest;
                        bounds_Lbest = frontline_bounds_Lbest;
                    elseif role == 2
                        bounds_Gbest = support_bounds_Gbest;
                        bounds_Lbest = support_bounds_Lbest;
                    else
                        bounds_Gbest = range_bounds_Gbest;
                        bounds_Lbest = range_bounds_Lbest;
                    end
                    
                    % Gbest
                    pop(j).bound = bounds_Gbest;                    
                    pop(j).Position = bounds_Gbest(1) + rand(1, nVar) .* (bounds_Gbest(2) - bounds_Gbest(1));
                    pop(j).Cost = CostFunction(pop(j).Position);
                    
                    % Lbest
                    pop(j+4).bound = bounds_Lbest;                    
                    pop(j+4).Position = bounds_Lbest(1) + rand(1, nVar) .* (bounds_Lbest(2) - bounds_Lbest(1));
                    pop(j+4).Cost = CostFunction(pop(j+4).Position);
