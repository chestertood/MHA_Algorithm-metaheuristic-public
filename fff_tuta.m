...
% GA
disp('Running GA...');
[BestSol_GA, BestCost_GA, Conv_GA] = GA(problem, params,InitialFitness);

% PSO
disp('Running PSO...');
[BestSol_PSO, BestCost_PSO, Conv_PSO] = PSO(problem, params,InitialFitness);

% GWO
disp('Running GWO...');
[BestSol_GWO, BestCost_GWO, Conv_GWO] = GWO(problem, params,InitialFitness);

% ABC
disp('Running ABC...');
[BestSol_ABC, BestCost_ABC, Conv_ABC] = ABC(problem, params,InitialFitness);

% SA
disp('Running SA...');
[BestSol_SA, BestCost_SA, Conv_SA] = SA(problem, params,InitialFitness);

% MHO
disp('Running MHO...');
[BestSol_MHO, BestCost_MHO, Conv_MHO] = MHO(problem, params,InitialFitness);

% WO
disp('Running WO...');
[Best_Score, Best_Pos, Convergence_curve_WO] = WO_NEW(problem, params,InitialFitness);

%Puma
disp('Running Puma...');
[Puma_X, Puma_C, Convergence_PUMA] = Puma_new(params, problem,InitialFitness);

disp('Running HBO...');
[Leader_score, Leader_pos, Convergence_curve_HBO] = HBO(problem, params,InitialFitness);

disp('Running WOA...');
[Leader_score_WOA, Leader_pos_WOA, Convergence_curve_WOA] = WOA(problem, params,InitialFitness);
...
semilogy(0:params.MaxIt,Conv_GA,'-r','LineWidth',1.5, 'DisplayName','GA');
hold on;
semilogy(0:params.MaxIt,Conv_PSO,'-b','LineWidth',1.5, 'DisplayName','PSO');
semilogy(0:params.MaxIt,Conv_GWO,'-g','LineWidth',1.5, 'DisplayName','GWO');
semilogy(0:params.MaxIt,Conv_ABC,'-k','LineWidth',1.5, 'DisplayName','ABC');
semilogy(0:params.MaxIt,Conv_SA,'-m','LineWidth',1.5, 'DisplayName','SA');
...
semilogy(0:params.MaxIt,Convergence_PUMA,'LineWidth',1.5, 'DisplayName','PUMA');
semilogy(0:params.MaxIt,Convergence_curve_HBO,'LineWidth',1.5, 'DisplayName','HBO');
semilogy(0:params.MaxIt,Convergence_curve_WOA,'LineWidth',1.5, 'DisplayName','WOA');
...
%% GA Function
function [BestSol, BestCost, Convergence] = GA(problem, params,InitialFitness)
    ...
    Convergence(1) = min(InitialFitness);
    for it = 1:MaxIt
        % Crossover
...
function y = GA_Mutate(x, mu, VarMin, VarMax)
    ...
    y = x;
    y(j) = unifrnd(VarMin, VarMax, size(j));
end

%% PSO Function
function [BestSol, BestCost, Convergence] = PSO(problem, params,InitialFitness)
    % Extract Problem Info
    CostFunction = problem.CostFunction;
    nVar = problem.nVar;
    VarMin = problem.VarMin;
...
%% GWO Function
function [BestSol, BestCost, Convergence] = GWO(problem, params,InitialFitness)
    % Extract Problem Info
    CostFunction = problem.CostFunction;
    nVar = problem.nVar;
    VarMin = problem.VarMin;
...
%% ABC Function
function [BestSol, BestCost, Convergence] = ABC(problem, params,InitialFitness)
    % Extract Problem Info
    CostFunction = problem.CostFunction;
    nVar = problem.nVar;
    VarMin = problem.VarMin;
...
%% SA Function
function [BestSol, BestCost, Convergence] = SA(problem, params,InitialFitness)
    % Extract Problem Info
    CostFunction = problem.CostFunction;
    nVar = problem.nVar;
    VarMin = problem.VarMin;
...
function [BestSol, BestCost, Convergence] = MHO(problem, params,InitialFitness)
...
function [Best_Score, Best_Pos, Convergence_curve_WO] = WO_NEW(problem,params,InitialFitness)
    ...
    Convergence_curve_WO = zeros(1, Maxit+1) ;  
    Convergence_curve_WO(1) = min(InitialFitness);
...
function [Puma_X, Puma_C, Convergence_PUMA] = Puma_new(params, problem,InitialFitness)
    % Extract parameters
    ...
    Convergence_PUMA = zeros(MaxIter+1, 1);
    Convergence_PUMA(1) = min(InitialFitness);
    % Unexperienced Phase
    for Iter = 1:3
        ...
        Sol = Sol(sind(1:nSol));
        Best = Sol(1);
        Convergence_PUMA(Iter+1) = Best.Cost; %#ok
    ...
    for Iter = 4:MaxIter
        ...
        % Store Best Cost
        Convergence_PUMA(Iter+1) = Best.Cost;
        disp(['Iteration: ' num2str(Iter) ' Best Cost = ' num2str(Best.Cost)]);
    end
...
function [Leader_score, Leader_pos, Convergence_curve_HBO] = HBO(problem, params,InitialFitness)
    ...
    % Main loop
    Convergence_curve_HBO = zeros(1, Max_iter+1);
    Convergence_curve_HBO(1) = min(InitialFitness);
    ...
    for it = 1:Max_iter
        ...
        Convergence_curve_HBO(it+1) = Leader_score;
        [fevals, Leader_score];
    end
end
...
function [Leader_score_WOA, Leader_pos_WOA, Convergence_curve_WOA] = WOA(problem, params,InitialFitness)
    ...
    Convergence_curve_WOA(1) = min(InitialFitness);
    t = 1; % Loop counter
    ...
    while t < Max_iter
        ...
        t = t + 1;
        Convergence_curve_WOA(t+1) = Leader_score_WOA;
        disp([t+1, Leader_score_WOA]);
    end
end