...
F = 'F9'; % Change this to any function ID from 'F1' to 'F23'
...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    distance_step = 0.08; % [0 - 1] ระยะห่างที่ทุกตัวเคลื่อนที่เข้าหา gbest 
    %npoint_before_go_deep = 600; % 0 - npoint ตัวอย่าง iter = 30 npop = 10 number_hunter_each_party = 4  : 30*10*4 = 1200 จุด
   
    
    % Algorithm Parameters
    MaxIt = params.MaxIt;           % Maximum Number of Iterations
    nPop = round(params.nPop/Number_hunter_each_party);             % Population Size params.nPop
    ...
    roles = {'roaming', 'frontline', 'support', 'range'};
    bounds = {roaming_bounds, frontline_bounds, support_bounds, range_bounds};
    n=5;
    ...
    for j = 1:Number_hunter_each_party
        role = mod(j - 1, Number_hunter_each_party) + 1; % 1=Roaming, 2=Frontline, 3=Support, 4=Range
        ...
        for dims = 1:nVar
            range_choice = randi([1, 2]); % Choose between the two ranges (1 or 2)
           
            if isequal(size(current_bounds), [2, 2])
                % Select bounds for the current dimension
                if range_choice == 1                
                    min_bound = current_bounds(1, 1);
                    max_bound = current_bounds(1, 2);
                else
                    min_bound = current_bounds(2, 1);
                    max_bound = current_bounds(2, 2);
                end
            else
                if range_choice == 1                
            ...
            pop(j).Position(dims) = min_bound + rand() * (max_bound - min_bound);
            
        end

        
        % Evaluate cost for the current position
        pop(j).role = role_choose;
        pop(j).bound = current_bounds;
    ...
    for u = 2:MaxIt                   
        for i = 1:nPop  
            ...
            % Define lengths for each role's bounds
            if n <= (((npoint*1)/4))                
                if Number_hunter_each_party == 4
                    roles = {'roaming', 'frontline', 'support', 'range'};
                    bounds = {roaming_bounds, frontline_bounds, support_bounds, range_bounds};
                end
            else 
                if isequal(size(VarMin), [1, 1])
                    VarMin_new = bounds{best_bound_index}(1, 1);
                    VarMax_new = bounds{best_bound_index}(2, 2);
                    midle_value = (VarMax + VarMin) / 2; 
                    roaming_bounds = [0.25 * VarMin_new, midle_value; midle_value, 0.25 * VarMax_new];
                    frontline_bounds = [0.5 * VarMin_new, 0.25 * VarMin_new; 0.25 * VarMax_new, 0.5 * VarMax_new];
                    support_bounds = [0.75 * VarMin_new, 0.5 * VarMin_new; 0.5 * VarMax_new, 0.75 * VarMax_new];
                    range_bounds = [VarMin_new, 0.75 * VarMin_new; 0.75 * VarMax_new, VarMax_new];
                    roles = {'roaming', 'frontline', 'support', 'range'};
                    bounds = {roaming_bounds, frontline_bounds, support_bounds, range_bounds};
                else
                    reduce_bound = struct();
                for n_reduce_bound =1:nVar
                    reduce_bound(n_reduce_bound).roaming_bounds = [];
                    reduce_bound(n_reduce_bound).frontline_bounds = [];
                    reduce_bound(n_reduce_bound).support_bounds = [];
                    reduce_bound(n_reduce_bound).range_bounds = [];
                    VarMax = usedInRange(n_reduce_bound).max;
                    VarMin = usedInRange(n_reduce_bound).min;
                    midle_value = (VarMax+VarMin)/2 ;      
                    range_min_to_mid = abs(VarMin-midle_value);
                    range_max_to_mid = abs(VarMax-midle_value);
                    VarMin_new = midle_value - range_min_to_mid ;
                    ...
                    support_bounds = [midle_value - 0.75 * range_min_to_mid, midle_value - 0.50 * range_min_to_mid; 0.50 * range_max_to_mid + midle_value, 0.75 * range_max_to_mid + midle_value];
                    range_bounds = [VarMin_new, midle_value - 0.75 * range_min_to_mid; 0.75 * range_max_to_mid + midle_value, VarMax_new];
                    reduce_bound(n_reduce_bound).roaming_bounds = roaming_bounds;
                    reduce_bound(n_reduce_bound).frontline_bounds = frontline_bounds;
                    reduce_bound(n_reduce_bound).support_bounds = support_bounds;
                    reduce_bound(n_reduce_bound).range_bounds = range_bounds;
            ...
            for j = 1:Number_hunter_each_party
                ...
                for dims = 1:nVar
                    ...
                    %var_max_new = bounds{best_bound_index}(dims,)
                    if isequal(size(current_bounds), [2, 2])
                        % Select bounds for the current dimension
                        if range_choice == 1                
                            min_bound = current_bounds(1, 1);
                        ...
                        if (n>((npoint*1)/4))
                            bound = {reduce_bound(dims).roaming_bounds,...
                                    reduce_bound(dims).frontline_bounds,...
                                     reduce_bound(dims).support_bounds,...
                                     reduce_bound(dims).range_bounds};
                            cb_reduce = bound{role};
                            
                            if range_choice == 1                
                                min_bound = cb_reduce(1, 1);
                                max_bound = cb_reduce(1, 2);
                            else
                                min_bound = cb_reduce(2, 1);
                                max_bound = cb_reduce(2, 2);
                            end
                        else
                    ...
                    % Randomly generate a position for the current dimension
                    pop(n).Position(dims) = min_bound + rand() * (max_bound - min_bound);
                ...
                % Evaluate cost for the current position
                pop(n).role = role_choose;
                pop(n).bound = current_bounds;
                pop(n).Cost = CostFunction(pop(j).Position);
                 % Display the hunter information
                n = n+1;
            end
            ...
            update_index  = 1;
            while  update_index ~= (length(costs)+1)
                best_pos = pop(bestIndex).Position;
                if update_index == bestIndex
                    update_index = update_index+1;
                    continue;        

                else                       
                    
                    delta = best_pos - pop(update_index).Position;                        
                    update_new_pos =  (pop(update_index).Position + (distance_step*delta));
                    pop(update_index).Position = update_new_pos;
                    pop(update_index).Cost = CostFunction(pop(update_index).Position);
            ...
            mean_pos_non = mean(positions);

            
            if isequal(size(VarMin), [1, 1])
                for best_bound_index=1:Number_hunter_each_party                   
                    
                    isInRange = checkInRange(mean_pos,best_bound_index, bounds);
                    if isInRange == 1                       
                        break
                    end
                end
                ...
                usedInRange = struct();
                for find_index = 1:nVar
                    usedInRange(find_index).choose_bound = [];
                    usedInRange(find_index).max = [];
                    usedInRange(find_index).min = [];
                    chose_mean_dim = mean_pos_non(find_index);
                    for best_bound_index=1:Number_hunter_each_party
                        chose_bound = bounds{best_bound_index};
                        
                        % ตรวจสอบ isInRange ทั้งสองเงื่อนไข
                        isInRange_min_to_mid = (chose_mean_dim >= chose_bound(1, find_index) && chose_mean_dim <= chose_bound(1, find_index + nVar));
                        isInRange_max_to_mid = (chose_mean_dim >= chose_bound(2, find_index) && chose_mean_dim <= chose_bound(2, find_index + nVar));
                        
                        % ถ้าไม่มีอันไหนเป็นจริง (ทั้ง 2 เป็น false)
                        % เก็บข้อมูลว่าใช้ตัวไหน
                        if isInRange_min_to_mid
                            usedInRange(find_index).choose_bound = chose_bound;
                            usedInRange(find_index).max_min = 'min_to_mid';
                            usedInRange(find_index).max = chose_bound(1, find_index + nVar);
                            usedInRange(find_index).min = chose_bound(1, find_index);
                        elseif isInRange_max_to_mid
                            usedInRange(find_index).choose_bound =chose_bound;
                            usedInRange(find_index).max_min = 'max_to_mid';
                            usedInRange(find_index).max = chose_bound(2, find_index + nVar);
                            usedInRange(find_index).min = chose_bound(2, find_index);
                        else
