...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    for u = 1:Num_iter
        ...
        sigma = 0.1;  % Step size (สามารถปรับค่าได้)
        alpha = 0.01;   % Step size for Lévy flight
        beta = 1.5;     % Lévy exponent (กำหนดให้เหมาะสม)
        run_1max = 0.9;
        run_1min = 0.2;
        run_1 = run_1max - (run_1max-run_1min)*u/Num_iter;
        run_2 = run_1min + (run_1max-run_1min)*u/Num_iter;

        for i = 1:sum_g1
            if rand() < 0.8  % ควบคุมว่าตัวไหนจะเคลื่อนที่
                step = sigma * randn(size(pop(i).Position));  % Gaussian step
                range_mid_bound = mid_bound-pop(i).Position;
                new_pos = pop(i).Position + run_1*(pop(i).Best.Position - pop(i).Position ) + run_2*(GlobalBest.Position -pop(i).Position) +0.05*range_mid_bound + step ;
%                
        ...
        for i = sum_g1+1:nPop
            if rand() < 0.8  % ควบคุมว่าตัวไหนจะเคลื่อนที่
                step = sigma * randn(size(pop(i).Position));  % Gaussian step
                if norm(VarMax - pop(i).Position) < norm(VarMin - pop(i).Position)
                    range_mid_bound = VarMax-pop(i).Position;
                    new_pos = pop(i).Position+0.3*rand()*range_mid_bound + step;
                    pop(i).Position = new_pos;
                else
                    range_mid_bound = VarMin-pop(i).Position;
                    new_pos = pop(i).Position+0.3*rand()*range_mid_bound + step;
                    pop(i).Position = new_pos;
...
%        maxVals = max(points, [], 1);
       ...
       for i = matches_support
           ...
           new_pos_plus = pos + (rand()+0.1)*Try_10per_bound_plus;
           new_pos_minus = pos + (rand()+0.1)*Try_10per_bound_minus;
