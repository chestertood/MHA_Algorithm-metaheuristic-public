...
% F = 'F14'; % Change this to any function ID from 'F1' to 'F23'
...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    safety_sum = 0;
    stun_gauge = 10 ;
    range_weapon = 0.08*(VarMax - VarMin); 



    pop = struct('role', [], 'Position', [], 'Cost', [] , 'Dim_chose',[]);
    ...
    for u = 1:MaxIt
        ...
        for i = matches_roaming            
            pos = pop(i).Position;     
            Delta1 = pos + 0.3*(GlobalBest.Position - pos);
            Test_Delta1 = Delta1;  
     ...
     for i = matches_range
         ...
         if safety_sum > stun_gauge %บอสติดสตั้น เลยเข้ามาตีใกล้ๆ
             new_pos_plus = Gbest +  (0.2*rand()+0.1) .* (pos - Gbest) ;
             new_pos_minus = Gbest -  (0.2*rand()+0.1) .* (pos - Gbest) ;
     ...
     safety_sum = safety_sum + safety_factor;
       ...
       for i = matches_support
           ...
           if rand() < 0.5
               new_pos = GlobalBest.Position + (0.4*rand()+0.1).*distance_from_gbest;
           else
               new_pos = GlobalBest.Position - (0.4*rand()+0.1).*distance_from_gbest;
           end
