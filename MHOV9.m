...
F = 'F5'; % Change this to any function ID from 'F1' to 'F23'
...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    Hunter = struct('role', [], 'Position', [], 'Cost', [] , 'Best',struct('Position',[],'Cost',[]));
    
    role = {'Blademaster' , 'Tank' , 'Supporter' , 'Ranger'};
    ...
    Hunter1st_rank = Hunter(1);
    
    matches_Blademaster = find(cellfun(@(b) isequal(b, 'Blademaster'), {Hunter.role}));          
    matches_Tank = find(cellfun(@(b) isequal(b, 'Tank'), {Hunter.role}));          
    matches_range = find(cellfun(@(b) isequal(b, 'range'), {Hunter.role}));          
    ...
    for u = 1:Num_iter*2
        ...
        [~,index_sort] = sort([Hunter(matches_Blademaster).Cost],'ascend');
       
        range_Is = Max_Is - Min_Is;

        Is1 = Max_Is - range_Is*(u-(1))/((Num_iter*2)); %Is = inertial speed
        Is2 =  Min_Is + range_Is*(u-(1))/((Num_iter*2));
        
        
        best_Blademaster = Hunter(index_sort(1));
        for i = matches_Blademaster            
            pos = Hunter(i).Position;     
            Delta1 = pos + Is1*(best_Blademaster.Position - pos);            
            Test_Delta1 = Delta1 + Is2*(Hunter1st_rank.Position - Delta1) ;
            ...
            if rand() < 0.9
                for j = 1:nVar
                    ...
                    Hunter(i).Cost = CostFunction(Test_Delta1);
                end                
       ...
       [~,index_sort] = sort([Hunter(matches_Tank).Cost],'ascend');
       index_sort =index_sort+length(matches_Blademaster);
       best_Tank = Hunter(index_sort(1));    


       for i = matches_Tank            
            pos = Hunter(i).Position;     
            Delta1 = Is1*(best_Tank.Position - pos) + pos;
            Delta1 = Delta1 +Is2*(Hunter1st_rank.Position - Delta1);
            Test_Delta1 = Delta1;
            danger_factor = rand();            
            
            %ตีเช็ค
            if danger_factor > 0.5
                new_pos = Test_Delta1 ;
            ...
            %สำรวจต่อ            
                ...
                for j = 1:nVar
                    roll_direct = rand();
                    if roll_direct < 0.4   
                        sigma = 0.1 * abs(Hunter1st_rank.Position(j) - new_pos(j));
                        dim = new_pos(j) + randn() * sigma;
                        new_pos(j) = dim;                       
         ...
         %วงล้อมรอบจุดอ่อนที่ดีที่สุด เดี๋ยวมาหาใหม่ เอาวงกลมหุบกาง Range        
         

         Hunter_best = Hunter1st_rank.Position;
         range_weapon = 0.08*(VarMax - VarMin); %ระยะยิงไกลสุดของrange
         for i = matches_range
             safety_factor = rand();%ค่าความปลอดภัยของสถานการณ์
             pos = Hunter(i).Position;
             
           
             if safety_factor > 0.7 %บอสติดสตั้น เลยเข้ามาตีใกล้ๆ
                 new_pos_plus = Hunter_best +  (0.2*rand(1,nVar)+0.1) .* (pos - Hunter_best) ;
                 new_pos_minus = Hunter_best -  (0.2*rand(1,nVar)+0.1) .* (pos - Hunter_best) ;
             ...
             elseif safety_factor < 0.7 %บอสโจมตีไปมา ต้องทิ้งระยะเพื่อความปลอดภัย
                 new_pos_plus = Hunter_best + rand() * (range_weapon) ;
                 new_pos_minus = Hunter_best - rand() * (range_weapon) ;
                 cost_plus = CostFunction(new_pos_plus);
                 cost_minus = CostFunction(new_pos_minus);
                 if cost_plus < cost_minus
                     Hunter(i).Position = new_pos_plus ;
       ...
       for i = matches_Supporter
           pos = Hunter(i).Position;
           distance_from_Hunter_best = pos - Hunter1st_rank.Position;
           
           if rand() < 0.5
               new_pos = Hunter1st_rank.Position + (0.4*rand(1,nVar)+0.1).*distance_from_Hunter_best;
           else
               new_pos = Hunter1st_rank.Position - (0.4*rand(1,nVar)+0.1).*distance_from_Hunter_best;
           end
       ...
       %CostFunction
        ...
        costs = [Hunter.Cost];  % Array of costs for each population member
        ...
        % Update Global Best
        if Hunter(bestIndex).Cost < Hunter1st_rank.Cost
            Hunter1st_rank = Hunter(bestIndex);               
        end      
...
function [BestSol, BestCost, Convergence] = MHO(problem, params)
    ...
    frontline_Blademaster_ratio = 1; % New Blademaster group ratio
    frontline_ratio = 1;
    support_ratio = 1;
    long_range_ratio = 1;
    ...
    total_ratio = frontline_ratio + support_ratio + long_range_ratio + frontline_Blademaster_ratio;
    frontline_Blademaster = round(nPop * (frontline_Blademaster_ratio / total_ratio)); % Remaining Hunters assigned to Blademaster
    ...
    for it = 1:MaxIt
        ...
        for i = 1:nPop
            ...
            elseif i <= frontline + frontline_Blademaster
                % Frontline Hunters (PSO-like movement)
            ...
            elseif i <= frontline + support + frontline_Blademaster
            ...
            %else  
            elseif i <= frontline + support + long_range + frontline_Blademaster
