...
load('optimization_results_23poblem.mat');
...
if exist('results', 'var') && isstruct(results)
    ...
    for p = 1:numel(results)
        ...
        for alg = tempAlgorithms
            ...
            BestCosts = [BestCosts; results(p).(alg).Mean_value]; %#ok<AGROW> %add
        ...
        ranks = zeros(size(BestCosts));
        for i = 1:length(BestCosts)
            rank = 1;  % Start with rank 1 for each BestCost
            for j = 1:length(BestCosts)
                if BestCosts(i) > BestCosts(j)
