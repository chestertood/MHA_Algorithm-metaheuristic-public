...
% Load results file
load('optimization_results_23poblem_30runs.mat');

filename = 'Optimization_Comparison100iter.xlsx';
...
algorithms = {'GA', 'PSO', 'GWO','ACOR', 'SA', 'MHO', 'WO', 'Puma', 'HBO', 'WOA','SWO'};
...
for algIdx = 1:numel(algorithms)
    ...
    for p = 1:numel(results)
        ...
        if strcmp(problemName, 'F10')
        [lb, ub, dim, fobj] = Functions(problemName); % Extract problem details
        
        % Initialize arrays for algorithm metrics
        tempAlgorithms = {'GA', 'PSO', 'GWO','ACOR', 'SA', 'MHO', 'WO', 'Puma', 'HBO', 'WOA','SWO'};
    ...
    SummaryTable = table(Problems, MinValues,BestcostsRanks,MaxValues,WorstcostsRanks, StdValues, ...
        StdRank,MeanValues, CostRanks, Times, TimeRanks, ...
        'VariableNames', {'Problem_Dimension', 'Bestcost','BestcostsRanks', 'Worstcost','WorstcostsRanks', 'Std', ...
                          'StdRank','Mean', 'MeanRank', 'CPUTime', 'CPUTimeRank'});
    
    % Display the summary table
    if strcmp(algName, 'MHO')
        disp(SummaryTable);
    end
