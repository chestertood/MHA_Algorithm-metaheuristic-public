...
for d = 1:numel(Dims)
    for p = 1:numel(Problems)
        ...
        results(d,p).Problem = i;  % Save problem identifier
        results(d,p).GA = aggregate_results(all_results.GA);
        results(d,p).PSO = aggregate_results(all_results.PSO);
        results(d,p).GWO = aggregate_results(all_results.GWO);
    %     results(p).ABC = aggregate_results(all_results.ABC);
        results(d,p).SA = aggregate_results(all_results.SA);
        results(d,p).MHO = aggregate_results(all_results.MHO);
        results(d,p).WO = aggregate_results(all_results.WO);
        results(d,p).Puma = aggregate_results(all_results.Puma);
        results(d,p).HBO = aggregate_results(all_results.HBO);
        results(d,p).WOA = aggregate_results(all_results.WOA);
        results(d,p).ACOR = aggregate_results(all_results.ACOR);
        results(d,p).SWO = aggregate_results(all_results.SWO);
    end
...
% Save all results
save('optimization_results_13poblem_30runs_alldim.mat', 'results');
