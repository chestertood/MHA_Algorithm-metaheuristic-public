...
%         all_results.ABC = [all_results.ABC; struct('BestCost', BestCost_ABC, 'Convergence', Conv_ABC,'Cpu_times', cpuTimeABC)];
...
% Save all results
save('optimization_results_23poblem_30ENruns.mat', 'results');
...
function aggregated = aggregate_results(runs)
    ...
    aggregated.Convergence = mean(convergences, 1); % Average convergence curve
end
...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    safety_sum = 0;
    stun_gauge = 10 ;
    range_weapon = 0.05*(VarMax - VarMin); 
    ...
    for u = 1:MaxIt
       ...
       for i = matches_frontline            
            pos = Hunter(i).Position;     
            IS2 = max(0.1, min(1, IS2));  % ป้องกันไม่ให้ IS2 ใหญ่เกินไป
