...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    range_weapon = 0.08*(VarMax - VarMin); %เอาออกมา จะได้คำนวณทีเดียว  
    Max_run = 0.9;
    Min_run = 0.2;
    safety_sum = 0;
    stun_gauge = 10 ;
    ...
    pop = struct('role', [], 'Position', [], 'Cost', [] , 'Best',struct('Position',[],'Cost',[]));
    ...
    for i = 1:nPop
        pop(i).Position = positions(i, :);  % นำตำแหน่งที่เรียงแล้วใส่ใน pop
        pop(i).Cost = CostFunction(pop(i).Position);  % เก็บค่า cost ที่เรียงแล้ว  
        pop(i).Best.Cost = inf;
    ...
    for u = 1:MaxIt
        ...
        %เคลื่อนที่ด้วยตัวเอง roaming    (ถูกแล้ว)      
                       
        
       
        range_run = Max_run - Min_run;

        run1 = Max_run - range_run*(u-1)/((MaxIt));
        run2 =  Min_run + range_run*(u-1)/((MaxIt));
        ...
        for i = matches_roaming            
            pos = pop(i).Position;     
            Delta1 = pos + run2*(GlobalBest.Position - pos);
            Test_Delta1 = Delta1;  
            
            
            %สำรวจจากจุดตัวเองไปกึ่งกลาง
            if rand() < 0.5
                for j = 1:nVar
                    dim = Test_Delta1(j);
                ...
                pop(i).Position = Test_Delta1; %ยัดนอก                
                pop(i).Cost = CostFunction(pop(i).Position); %ยัดนอก  
            ...
            %สำรวจรอบGbest
               ...
               for j = 1:nVar
                    dim = Test_Delta1(j);
                    range_mid = GlobalBest.Position(j) - dim;
                    opposie_dim = GlobalBest.Position(j)+range_mid;                     
       ...
       %เคลื่อนที่ด้วย frontline  (ควรหาการคทoriginalของตัวเอง)  (ถูกแล้ว)        
       ...
       for i = matches_frontline            
            ...
            %สำรวจต่อ            
                ...
                for j = 1:nVar
                    roll_direct = rand();
                    if roll_direct < 0.4   
                        sigma = 0.1 * abs(GlobalBest.Position(j) - new_pos(j));
         ...
         %วงล้อมรอบจุดอ่อนที่ดีที่สุด เดี๋ยวมาหาใหม่ เอาวงกลมหุบกาง Range (มีแก้)       
         ...
         for i = matches_range
             ...
             if safety_sum > stun_gauge %บอสติดสตั้น เลยเข้ามาตีใกล้ๆ
                 new_pos_plus = Gbest +  (0.2*rand(1,nVar)+0.1) .* (pos - Gbest) ; %ลองเปลี่ยนเป็นrand()เวลาน่าจะดีขึ้น
                 new_pos_minus = Gbest -  (0.2*rand(1,nVar)+0.1) .* (pos - Gbest) ;%ลองเปลี่ยนเป็นrand()เวลาน่าจะดีขึ้น
                 new_pos_plus = max(VarMin, min(VarMax, new_pos_plus)); %check ก่อนเข้าCostfunc
                 new_pos_minus = max(VarMin, min(VarMax, new_pos_minus));  %check ก่อนเข้าCostfunc
                 cost_plus = CostFunction(new_pos_plus);
                 cost_minus = CostFunction(new_pos_minus);
                 
                 if cost_plus < cost_minus && cost_plus<pop(i).Cost
                     pop(i).Position = new_pos_plus ;
                 elseif cost_minus < cost_plus && cost_minus<pop(i).Cost
                     pop(i).Position = new_pos_minus;                     
                 end
                 
             elseif safety_sum < stun_gauge %บอสโจมตีไปมา ต้องทิ้งระยะเพื่อความปลอดภัย
                 new_pos_plus = Gbest + rand() * (range_weapon) ;
                 new_pos_minus = Gbest - rand() * (range_weapon) ;
                 new_pos_plus = max(VarMin, min(VarMax, new_pos_plus)); %check ก่อนเข้าCostfunc
                 new_pos_minus = max(VarMin, min(VarMax, new_pos_minus)); %check ก่อนเข้าCostfunc
                 cost_plus = CostFunction(new_pos_plus);
                 cost_minus = CostFunction(new_pos_minus);
                 safety_sum = safety_sum + safety_factor; %plus in loop หรือเอาแบบเดิมก็ได้ค่อยคุย
                 if cost_plus < cost_minus && cost_plus<pop(i).Cost
                     pop(i).Position = new_pos_plus ;
                 elseif cost_minus < cost_plus && cost_minus<pop(i).Cost
                     pop(i).Position = new_pos_minus;
       ...
       %เคลื่อนที่ด้วย support (ถูกแล้ว)   อาจเปลี่ยนแปลงทีหลัง  
       
    
       for i = matches_support
           pos = pop(i).Position;
           distance_from_gbest = pos - GlobalBest.Position;
           
           if rand() < 0.5
               new_pos = GlobalBest.Position + (0.4*rand(1,nVar)+0.1).*distance_from_gbest;
           else
               new_pos = GlobalBest.Position - (0.4*rand(1,nVar)+0.1).*distance_from_gbest;
           end
