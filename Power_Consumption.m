...
% กำหนดค่าพารามิเตอร์
numMachines = 5;  % จำนวนเครื่องจักร
timeSlots = 24;   % จำนวนช่วงเวลา
powerUsage = [5, 8, 6, 7, 9];  % การใช้พลังงานแต่ละเครื่อง (kW)

% ฟังก์ชันเป้าหมาย: ลดพลังงานสูงสุด
objective = @(x) maxPowerConsumption(x, numMachines, timeSlots, powerUsage);

% กำหนดขอบเขตตัวแปร (แต่ละเครื่องมีค่าได้ตั้งแต่ 1 ถึง 24)
lb = ones(1, numMachines);
ub = timeSlots * ones(1, numMachines);

% กำหนดพารามิเตอร์สำหรับอัลกอริทึมต่าง ๆ
problem.CostFunction = objective;
problem.nVar = numMachines;
problem.VarMin = lb;
problem.VarMax = ub;
params.MaxIt = 100;
params.nPop = 50;
...
% ใช้ทุกอัลกอริทึมเพื่อหาคำตอบ
disp('Running MHO...');
[BestSol_MHO, BestCost_MHO, ~] = MHO_Adaptive(problem, params);

disp('Running GA...');
[BestSol_GA, BestCost_GA, ~] = GA(problem, params);

disp('Running PSO...');
[BestSol_PSO, BestCost_PSO, ~] = PSO(problem, params);

disp('Running GWO...');
[BestSol_GWO, BestCost_GWO, ~] = GWO(problem, params);

disp('Running SA...');
[BestSol_SA, BestCost_SA, ~] = SA(problem, params);

disp('Running WO...');
[BestSol_WO, BestCost_WO, ~] = WO_NEW(problem, params);

disp('Running Puma...');
[BestSol_Puma, BestCost_Puma, ~] = Puma_new(params, problem);

disp('Running HBO...');
[BestSol_HBO, BestCost_HBO, ~] = HBO(problem, params);

disp('Running WOA...');
[BestSol_WOA, BestCost_WOA, ~] = WOA(problem, params);

disp('Running ACOR...');
[BestCost_ACOR, BestSol_ACOR, ~] = ACOR(problem, params);

% แสดงผลลัพธ์จากทุกอัลกอริทึม
algorithms = {'MHO', 'GA', 'PSO', 'GWO', 'SA', 'WO', 'Puma', 'HBO', 'WOA', 'ACOR'};
bestSolutions = {BestSol_MHO, BestSol_GA, BestSol_PSO, BestSol_GWO, BestSol_SA, BestSol_WO, BestSol_Puma, BestSol_HBO, BestSol_WOA, BestSol_ACOR};
bestCosts = [BestCost_MHO, BestCost_GA, BestCost_PSO, BestCost_GWO, BestCost_SA, BestCost_WO, BestCost_Puma, BestCost_HBO, BestCost_WOA, BestCost_ACOR];

for i = 1:numel(algorithms)
    fprintf('%s Best Solution: %s\n', algorithms{i}, mat2str(bestSolutions{i}));
    fprintf('%s Minimum Peak Power: %.2f\n', algorithms{i}, bestCosts(i));
end

% แสดงผลลัพธ์ในรูปแบบตาราง
fprintf('\n------ Algorithm Rankings ------\n');
fprintf('Algorithm | Minimum Peak Power |      Best Schedule\n');
fprintf('----------|--------------------|-------------------------\n');
for i = 1:numel(algorithms)
    % แปลงเวลาที่เครื่องจักรทำงานเป็นจำนวนเต็ม
    schedule = round(bestSolutions{i});
    
    % หาก Best Solution เป็นสเกลาร์ ให้แปลงเป็นเวกเตอร์
    if isscalar(schedule)
        schedule = repmat(schedule, 1, numMachines); % แปลงเป็นเวกเตอร์
    end
    
    schedule_str = mat2str(schedule);
    
    fprintf('%9s | %17.2f kW | %s\n', ...
            algorithms{i}, bestCosts(i), schedule_str);
end
fprintf('----------|--------------------|-------------------------\n');

% ---------------- ฟังก์ชันคำนวณพลังงานสูงสุด ----------------
function peakPower = maxPowerConsumption(schedule, numMachines, timeSlots, powerUsage)
    powerDistribution = computePowerDistribution(schedule, numMachines, timeSlots, powerUsage);
    peakPower = max(powerDistribution);
end

% ---------------- ฟังก์ชันคำนวณโหลดพลังงาน ----------------
function powerDistribution = computePowerDistribution(schedule, numMachines, timeSlots, powerUsage)
    powerDistribution = zeros(1, timeSlots);
    for i = 1:numMachines
        timeSlot = round(schedule(i));
        powerDistribution(timeSlot) = powerDistribution(timeSlot) + powerUsage(i);
    end
end

% ที่เหลือเป็นฟังก์ชันอัลกอริทึมต่าง ๆ เช่น MHO_Adaptive, GA, PSO, GWO, SA, WO_NEW, Puma_new, HBO, WOA, ACOR ที่อยู่ในโค้ดที่ 1
...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    for u = 1:MaxIt
        ...
        for i = matches_BladeMaster            
            ...
            Delta1 = pos + IS2*(Hunter1st_rank.Position - pos);
            Test_Delta1 = Delta1;  
            arive = rand();
            
%             if arive <0.65            
                %ตีเช็ค
                if u <= round(MaxIt*0.3)
                    for j = 1:nVar
                        dim = Test_Delta1(j);
...
%             else
%                 if rand() < 0.5
%                     Hunter(i).Position = VarMax;
%                     Hunter(i).CostFunction = Hunter(i).Position ;
%                 else
%                     Hunter(i).Position = VarMin;
%                     Hunter(i).CostFunction = Hunter(i).Position ;
%                 end
...
%             end
            ...
            cost_blade = [Hunter(matches_BladeMaster).Cost];
            [sortcost_blade,indexcost_blade] = sort(cost_blade,'ascend');
%             if itd == 20
%                 itd = 0;
%                 if rand() <0.5
%                     Hunter(indexcost_blade(13)).Position = VarMax;
%                     Hunter(indexcost_blade(13)).CostFunction = Hunter(i).Position ;
%                     Hunter(indexcost_blade(12)).Position = VarMax;
%                     Hunter(indexcost_blade(12)).CostFunction = Hunter(i).Position ;
%                 else
%                     Hunter(indexcost_blade(13)).Position = VarMin;
%                     Hunter(indexcost_blade(13)).CostFunction = Hunter(i).Position ;
%                     Hunter(indexcost_blade(12)).Position = VarMin;
%                     Hunter(indexcost_blade(12)).CostFunction = Hunter(i).Position ;
%                 end
%                 
%             end
       ...
       for i = matches_frontline            
            ...
            cost_front = [Hunter(matches_frontline).Cost];
            [sortcost_front,indexcost_front] = sort(cost_front,'ascend');
     ...
     for i = matches_Ranger
         ...
         cost_ranger = [Hunter(matches_Ranger).Cost];
         [sortcost_Ranger,indexcost_Ranger] = sort(cost_ranger,'ascend');
...
%            if rand < 0.8  %Alive               
               dim_choose = randi(nVar);
               dim_gbest =  Hunter1st_rank.Position(dim_choose);
               new_pos = dim_gbest.*ones(1,nVar);
               cost_new = CostFunction(new_pos);
           
               if cost_new < Hunter(i).Cost                  
                   Hunter(i).Position = new_pos;
                   Hunter(i).Cost = cost_new;
               end
...
%                if rand <0.5
%                    Hunter(i).Position = VarMax;
%                    Hunter(i).CostFunction = Hunter.Position;
%                else
%                    Hunter(i).Position = VarMin;
%                    Hunter(i).CostFunction = Hunter.Position;
%                end
%            end
           ...
           cost_sup = [Hunter(matches_Supporter).Cost];
           [sortcost_Sup,indexcost_Sup] = sort(cost_sup,'ascend');
       ...
       if itd == 20
            ...
            if rand() <0.5
                Hunter(indexcost_blade(13)).Position = VarMax;
                Hunter(indexcost_blade(13)).Cost = CostFunction(Hunter(indexcost_blade(13)).Position) ;
                Hunter(indexcost_blade(12)).Position = VarMax;
                Hunter(indexcost_blade(12)).Cost = CostFunction(Hunter(indexcost_blade(12)).Position) ;
            else
                Hunter(indexcost_blade(13)).Position = VarMin;
                Hunter(indexcost_blade(13)).Cost = CostFunction(Hunter(indexcost_blade(13)).Position) ;
                Hunter(indexcost_blade(12)).Position = VarMin;
            ...
            if rand() <0.5
                Hunter(indexcost_front(13)).Position = VarMax;
                Hunter(indexcost_front(13)).Cost = CostFunction(Hunter(indexcost_front(13)).Position) ;
                Hunter(indexcost_front(12)).Position = VarMax;
                Hunter(indexcost_front(12)).Cost = CostFunction(Hunter(indexcost_front(12)).Position) ;
            else
                Hunter(indexcost_front(13)).Position = VarMin;
                Hunter(indexcost_front(13)).Cost = CostFunction(Hunter(indexcost_front(13)).Position) ;
                Hunter(indexcost_front(12)).Position = VarMin;
                Hunter(indexcost_front(12)).Cost = CostFunction(Hunter(indexcost_front(12)).Position) ;
            end

            if rand() <0.5
                Hunter(indexcost_Ranger(11)).Position = VarMax;
                Hunter(indexcost_Ranger(11)).Cost = CostFunction(Hunter(indexcost_Ranger(11)).Position) ;
                Hunter(indexcost_Ranger(10)).Position = VarMax;
                Hunter(indexcost_Ranger(10)).Cost = CostFunction(Hunter(indexcost_Ranger(10)).Position) ;
            else
                Hunter(indexcost_Ranger(11)).Position = VarMin;
                Hunter(indexcost_Ranger(11)).Cost = CostFunction(Hunter(indexcost_Ranger(11)).Position) ;
                Hunter(indexcost_Ranger(10)).Position = VarMin;
                Hunter(indexcost_Ranger(10)).Cost = CostFunction(Hunter(indexcost_Ranger(10)).Position) ;
            end

            if rand() <0.5
                Hunter(indexcost_Sup(13)).Position = VarMax;
                Hunter(indexcost_Sup(13)).Cost = CostFunction(Hunter(indexcost_Sup(13)).Position) ;
                Hunter(indexcost_Sup(12)).Position = VarMax;
                Hunter(indexcost_Sup(12)).Cost = CostFunction(Hunter(indexcost_Sup(12)).Position) ;
            else
                Hunter(indexcost_Sup(13)).Position = VarMin;
                Hunter(indexcost_Sup(13)).Cost = CostFunction(Hunter(indexcost_Sup(13)).Position) ;
                Hunter(indexcost_Sup(12)).Position = VarMin;
                Hunter(indexcost_Sup(12)).Cost = CostFunction(Hunter(indexcost_Sup(12)).Position) ;
            end
       end
