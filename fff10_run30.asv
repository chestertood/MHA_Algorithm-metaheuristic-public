...
for p = 1:numel(Problems)
    ...
    all_results = struct('GA', [], 'PSO', [], 'GWO', [], 'ACOR', [], 'SA', [], 'MHO', [], 'WO', [], 'Puma', [], 'HBO', [], 'WOA', [],'SWO',[]);
    ...
    for run = 1:run_no
        ...
        all_results.ACOR = [all_results.ACOR; struct('BestCost', BestCost_ACO,'Convergence', Convergence_curve_ACOR, 'Cpu_times', cpuTimeACOR)];
...
% Save all results
save('optimization_results_23poblem_30runs.mat', 'results');
    ...
    for u = 1:MaxIt
        ...
        for i = matches_BladeMaster            
            ...
            if u <= round(MaxIt*0.3)
                for j = 1:nVar
                    ...
                    new_dim_plus = dim + (0.9*rand()+0.05)*(opposie_dim-dim);
                    new_dim_minus = dim - (0.9*rand()+0.05)*(opposie_dim-dim);
     ...
     safety_sum = safety_sum + safety_factor;
       ...
       for i = matches_Supporter
           ...
           if CostFunction(new_pos) < Hunter(i).Cost
...
%        matches_Supporter = find(cellfun(@(b) isequal(b, 'Supporter'), {Hunter.role}));
       ...
       if itd == 20
           [~,indexcost_blade] = sort([Hunter(matches_BladeMaster).Cost],'ascend');
           [~,indexcost_front] = sort([Hunter(matches_frontline).Cost],'ascend');
           [~,indexcost_Ranger] = sort([Hunter(matches_Ranger).Cost],'ascend');
           [~,indexcost_Sup] = sort([Hunter(matches_Supporter).Cost],'ascend');
            itd = 0;
            if rand() <0.5
                Hunter(indexcost_blade(end)).Position = VarMax;
                Hunter(indexcost_blade(end)).Cost = CostFunction(Hunter(indexcost_blade(end)).Position) ;
                Hunter(indexcost_blade(end-1)).Position = VarMax;
                Hunter(indexcost_blade(end-1)).Cost = CostFunction(Hunter(indexcost_blade(end-1)).Position) ;
            else
                Hunter(indexcost_blade(end)).Position = VarMin;
                Hunter(indexcost_blade(end)).Cost = CostFunction(Hunter(indexcost_blade(end)).Position) ;
                Hunter(indexcost_blade(end-1)).Position = VarMin;
                Hunter(indexcost_blade(end-1)).Cost = CostFunction(Hunter(indexcost_blade(end-1)).Position) ;
            end
            
            if rand() <0.5
                Hunter(indexcost_front(end)).Position = VarMax;
                Hunter(indexcost_front(end)).Cost = CostFunction(Hunter(indexcost_front(end)).Position) ;
                Hunter(indexcost_front(end-1)).Position = VarMax;
                Hunter(indexcost_front(end-1)).Cost = CostFunction(Hunter(indexcost_front(end-1)).Position) ;
            else
                Hunter(indexcost_front(end)).Position = VarMin;
                Hunter(indexcost_front(end)).Cost = CostFunction(Hunter(indexcost_front(end)).Position) ;
                Hunter(indexcost_front(end-1)).Position = VarMin;
                Hunter(indexcost_front(end-1)).Cost = CostFunction(Hunter(indexcost_front(end-1)).Position) ;
            end

            if rand() <0.5
                Hunter(indexcost_Ranger(end)).Position = VarMax;
                Hunter(indexcost_Ranger(end)).Cost = CostFunction(Hunter(indexcost_Ranger(end)).Position) ;
                Hunter(indexcost_Ranger(end-1)).Position = VarMax;
                Hunter(indexcost_Ranger(end-1)).Cost = CostFunction(Hunter(indexcost_Ranger(end-1)).Position) ;
            else
                Hunter(indexcost_Ranger(end)).Position = VarMin;
                Hunter(indexcost_Ranger(end)).Cost = CostFunction(Hunter(indexcost_Ranger(end)).Position) ;
                Hunter(indexcost_Ranger(end-1)).Position = VarMin;
                Hunter(indexcost_Ranger(end-1)).Cost = CostFunction(Hunter(indexcost_Ranger(end-1)).Position) ;
            end

            if rand() <0.5
                Hunter(indexcost_Sup(end)).Position = VarMax;
                Hunter(indexcost_Sup(end)).Cost = CostFunction(Hunter(indexcost_Sup(end)).Position) ;
                Hunter(indexcost_Sup(end-1)).Position = VarMax;
                Hunter(indexcost_Sup(end-1)).Cost = CostFunction(Hunter(indexcost_Sup(end-1)).Position) ;
            else
                Hunter(indexcost_Sup(end)).Position = VarMin;
                Hunter(indexcost_Sup(end)).Cost = CostFunction(Hunter(indexcost_Sup(end)).Position) ;
                Hunter(indexcost_Sup(end-1)).Position = VarMin;
                Hunter(indexcost_Sup(end-1)).Cost = CostFunction(Hunter(indexcost_Sup(end-1)).Position) ;
            end
       end
