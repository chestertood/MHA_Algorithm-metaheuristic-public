...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    distance_step = 0.2; % [0 - 1] ระยะห่างที่ทุกตัวเคลื่อนที่เข้าหา gbest 
    
    % Algorithm Parameters
    MaxIt = params.MaxIt;           % Maximum Number of Iterations
    nPop = params.nPop / Number_hunter_each_party; % Population Size params.nPop
    npoint = MaxIt * nPop * 4;
    ...
    if Number_hunter_each_party == 4 
        ...
        for j = 1:Number_hunter_each_party
            role = mod(j - 1, 4) + 1; % 1=Roaming, 2=Frontline, 3=Support, 4=Range
        ...
        for u = 2:MaxIt                   
            for i = 1:nPop  
                ...
                for best_bound_index = 1:4
                    isInRange = checkInRange(mean_pos, bounds{best_bound_index}(1, 1), bounds{best_bound_index}(1, 2), bounds{best_bound_index}(2, 1), bounds{best_bound_index}(2, 2));
                    if isInRange == 1 
                        break;
                    end
                end 
...
function [BestSol, BestCost, Convergence] = MHO(problem, params)
    ...
    support_bounds = [VarMin + 0.5 * total_range, VarMin + 0.75 * total range];
    ...
    % Compute detailed statistics for results
    max_val = max(Convergence);
    min_val = min(Convergence);
    mean_val = mean(Convergence);
    median_val = median(Convergence);
    std_val = std(Convergence);
    cpuTime = cputime;
    ...
    results = struct('BestSol', BestSol, 'BestCost', BestCost, 'Convergence', Convergence, ...
                     'Max_value', max_val, 'Min_value', min_val, 'Mean_value', mean_val, ...
                     'Std_value', std_val, 'Median_value', median_val, 'Cpu_times', cpuTime);
end
