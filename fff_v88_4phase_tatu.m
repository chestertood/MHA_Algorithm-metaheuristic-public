...
% Save all results
save('optimization_results_EN_poblem.mat', 'results');
...
%     positions = VarMin + sobolPoints .* (VarMax - VarMin);
    ...
    for u = 1:MaxIt 
        ...
        if  (u>10) && (u<=20)
            ...
            elseif (u >11) && (u<=20)
                for i = 1:3
                    total_pos = total_pos + pop(matches_roaming(index_sortroaming(i))).Position;
                    points = [points,pop(i).Position];
                end
           
            end
            mean_pos = (total_pos/3); 
            ...
            for i = matches_roaming
             ...
             cost_roaming = [cost_roaming,pop(i).Cost ];
            [sortedCost_roaming,index_sortroaming] = sort(cost_roaming,'ascend');
           ...
           run3 = Max_run - range_run*(u-11)/(20-11);
           run4 = Min_run + range_run*(u-11)/(20-11);
           sigma = 2 - 1*(u-11)/(20-11); %ค่าช่วง2กว้าง 1 ปกติ 0.5แคบ
%            sigma = (0.7 - (0.7-0.3)*(u-11)/(MaxIt-11))*2;
           ...
           for i = matches
                ...
                if healing_factor > 0.5 %focus on healing allies
        ...
        if (u>20) && (u<=22)
          ...
          for i =matches_frontline               
           ...
           points = reshape(points,nVar,20).';
           mean_pos = mean(points);
           run3 = Max_run - range_run*(u-21)/(22-21);
...
%            sigma = (0.7 - (0.7-0.3)*(u-11)/(MaxIt-11))*2;
           for i = matches
                ...
                pc = CostFunction(pos);
                gaussian_step = sigma * randn(1, nVar);                
                new_pos = GlobalBest.Position + pos .* levyFlight(nVar).* run3;              
                c = CostFunction(new_pos);
                if c < pc
                    pop(i).Position = new_pos;
                    pop(i).Cost = c; 
           ...
           points = reshape(points,nVar,20).';
           mean_pos = mean(points);
           sigma = 2 - (0.5)*(u-21)/(22-21); %ค่าช่วง2กว้าง 1 ปกติ 0.5แคบ
           run4 = 0.5 + 0.2*(u-21)/(22-21);
           for i = matches_range
               pos = pop(i).Position;
               pc = CostFunction(pos);
               gaussian_step = sigma * randn(1, nVar);
               new_pos = GlobalBest.Position + run4 .* gaussian_step;
               c = CostFunction(new_pos);
               if c < pc
                   pop(i).Position = new_pos;
                   if c<GlobalBest.Cost
