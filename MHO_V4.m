...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    Number_hunter_each_party = 4;
    weapon = "sword"; % [sword: y=x , gun : y=sin(x) , switch-axe: y=x^2]
    
     % [0 - 1] ระยะห่างที่ทุกตัวเคลื่อนที่เข้าหา gbest 
    %npoint_before_go_deep = 600; % 0 - npoint ตัวอย่าง iter = 30 npop = 10 number_hunter_each_party = 4  : 30*10*4 = 1200 จุด
   
    
    % Algorithm Parameters
    MaxIt = params.MaxIt;           % Maximum Number of Iterations
    nPop = round(params.nPop/8);             % Population Size params.nPop
    npoint = MaxIt*nPop*Number_hunter_each_party;

    wmax = 0.7;              % Inertia Weight
    wmin = 0.2;       % Damping Ratio
   ...
   c2 = 2.0;           % Social Acceleration
    ...
    midle_value = (VarMax+VarMin)/2 ;      
    range_min_to_mid = abs(VarMin-midle_value);
    range_max_to_mid = abs(VarMax-midle_value);
    VarMin_new = midle_value - range_min_to_mid ;
    VarMax_new = midle_value + range_max_to_mid;
    
    if ~isequal(size(midle_value),[1 1])       
    
        dim.role = [];    
        dim.bound = [];
        
        
        bound_per_dims = repmat(dim, nVar, 1);
    
        for i =1:nVar
            midle_value_loop = midle_value(i);
            range_min_to_mid = abs(VarMin(i)-midle_value(i));
            range_max_to_mid = abs(VarMax(i)-midle_value(i));
            VarMin_new = midle_value(i) - range_min_to_mid;
            VarMax_new = midle_value(i) + range_max_to_mid;
    
            roaming_bounds = [midle_value_loop - 0.25 * range_min_to_mid , midle_value_loop; midle_value_loop, 0.25 * range_max_to_mid + midle_value_loop];
            frontline_bounds = [midle_value_loop - 0.5 * range_min_to_mid, midle_value_loop - 0.25 * range_min_to_mid; 0.25 * range_max_to_mid + midle_value_loop, 0.5 * range_max_to_mid + midle_value_loop];
            support_bounds = [midle_value_loop - 0.75 * range_min_to_mid, midle_value_loop - 0.50 * range_min_to_mid; 0.50 * range_max_to_mid + midle_value_loop, 0.75 * range_max_to_mid + midle_value_loop];
            range_bounds = [VarMin_new, midle_value_loop - 0.75 * range_min_to_mid; 0.75 * range_max_to_mid + midle_value_loop, VarMax_new];
            roles = {'roaming','roaming', 'frontline','frontline', 'support','support', 'range','range'};
            bounds = {roaming_bounds(1,:),roaming_bounds(2,:), frontline_bounds(1,:),frontline_bounds(2,:), support_bounds(1,:),support_bounds(2,:), range_bounds(1,:),range_bounds(2,:)};
        
            bound_per_dims(i).role = roles;
            bound_per_dims(i).bound = bounds;
        end
    end
    % Define bounds for all roles
  

    roaming_bounds = [midle_value - 0.25 * range_min_to_mid , midle_value; midle_value, 0.25 * range_max_to_mid + midle_value];
    frontline_bounds = [midle_value - 0.5 * range_min_to_mid, midle_value - 0.25 * range_min_to_mid; 0.25 * range_max_to_mid + midle_value, 0.5 * range_max_to_mid + midle_value];
    support_bounds = [midle_value - 0.75 * range_min_to_mid, midle_value - 0.50 * range_min_to_mid; 0.50 * range_max_to_mid + midle_value, 0.75 * range_max_to_mid + midle_value];
    range_bounds = [VarMin_new, midle_value - 0.75 * range_min_to_mid; 0.75 * range_max_to_mid + midle_value, VarMax_new];
    
    % Define lengths for each role's bounds
    roles = {'roaming','roaming', 'frontline','frontline', 'support','support', 'range','range'};
    bounds = {roaming_bounds(1,:),roaming_bounds(2,:), frontline_bounds(1,:),frontline_bounds(2,:), support_bounds(1,:),support_bounds(2,:), range_bounds(1,:),range_bounds(2,:)};
    n=5;
    ...
    for mem_hunt=1:(nPop)
        for j = 1:Number_hunter_each_party*2
            role = mod(j - 1, Number_hunter_each_party*2) + 1; % 1=Roaming, 2=Frontline, 3=Support, 4=Range
            current_bounds = bounds{role};
            role_choose = roles{role};
            pop(num_2).Velocity = zeros(1, nVar);
            
            
            % Loop through each dimension and select the random value from the union of two ranges
            
            for dims = 1:nVar
                 % Choose between the two ranges (1 or 2)           
                if isequal(size(current_bounds), [1, 2])             
                                
                        min_bound = current_bounds(1, 1);
                        max_bound = current_bounds(1, 2);                 
    
                    
                else
                    if range_choice == 1                
                        min_bound = current_bounds(1, dims);
                        max_bound = current_bounds(1, dims+nVar);
                    else
                        min_bound = current_bounds(2, dims);
                        max_bound = current_bounds(2, dims+nVar);
                    end
                end
                
                
                % Randomly generate a position for the current dimension
                pop(num_2).Position(dims) = min_bound + rand() * (max_bound - min_bound);
                
                
            end
    
            
            % Evaluate cost for the current position
            pop(num_2).role = role_choose;
            pop(num_2).bound = current_bounds;
            pop(num_2).Cost = CostFunction(pop(num_2).Position);
             % Display the hunter information
    ...
    remainingCosts(bestIndex) = inf;  % Set the best cost to infinity to exclude it
    
    % Find the second-best cost
    [~, secondBestIndex] = min(remainingCosts);
    ...
    if pop(secondBestIndex).Cost < LocalBest.Cost
        LocalBest = pop(secondBestIndex);
    end
    ...
    %moving
    randInRange = @(minVal, maxVal) minVal + (maxVal - minVal) * rand();
    for u = 1:MaxIt   
        % เดินวนรอบๆ
        if u <=3
            ...
            bestcost_bound = repmat(cood, 8, 1);
            for i = 1:length(bounds)            
                targetBound = bounds{i};      
                matches = find(cellfun(@(b) isequal(b, targetBound), {pop.bound}));
                bestcost_bound(i).bound =  targetBound;
                bestcost_bound(i).index =  matches;
                if isempty(matches)
                    bestcost_perbound = -inf;
                    bestcost_bound(i).bestcost = bestcost_perbound;
                else                    
                   for j =1:length(matches)  
                    bestcost_bound(i).cost(j) =  pop(matches(j)).Cost;
                    end
                    bestcost_perbound = min([bestcost_bound(i).cost]);
                    bestcost_bound(i).bestcost = bestcost_perbound;
                end
            ...
            cost = [bestcost_bound.bestcost] ;            
    

            % Step 2: Find the index of the minimum cost
            [~, worstcost_1] = max(cost);
            
            
            remainingCosts = cost;
            remainingCosts(worstcost_1) = -inf;  % Set the best cost to infinity to exclude it
            
            % Find the second-best cost
            [~, worstcost_2] = max(remainingCosts); 

            remainingCosts = cost;
            remainingCosts(worstcost_1) = -inf;
            remainingCosts(worstcost_2) = -inf;  % Set the best cost to infinity to exclude it
            
            % Find the second-best cost
            [~, worstcost_3] = max(remainingCosts);

            remainingCosts = cost;
            remainingCosts(worstcost_1) = -inf;
            remainingCosts(worstcost_2) = -inf;  % Set the best cost to infinity to exclude it
            remainingCosts(worstcost_3) = -inf;
            
            % Find the second-best cost
            [~, worstcost_4] = max(remainingCosts);
            
            for g =1:length(bestcost_bound(worstcost_1).index)
                min_bound = bestcost_bound(worstcost_3).bound(1, 1);
                max_bound = bestcost_bound(worstcost_3).bound(1, 2);
                
                for dimss = 1:nVar
                    pop(bestcost_bound(worstcost_1).index(g)).Position(dimss)  = min_bound + rand() * (max_bound - min_bound);                    
                end 
                pop(bestcost_bound(worstcost_1).index(g)).Cost = CostFunction(pop(bestcost_bound(worstcost_1).index(g)).Position);
                pop(bestcost_bound(worstcost_1).index(g)).bound = bestcost_bound(worstcost_3).bound;                
                
            end
            for g =1:length(bestcost_bound(worstcost_2).index)
                min_bound = bestcost_bound(worstcost_4).bound(1, 1);
                max_bound = bestcost_bound(worstcost_4).bound(1, 2);
                
                for dimss = 1:nVar
                    pop(bestcost_bound(worstcost_2).index(g)).Position(dimss)  = min_bound + rand() * (max_bound - min_bound);                    
                end 
                pop(bestcost_bound(worstcost_2).index(g)).Cost = CostFunction(pop(bestcost_bound(worstcost_2).index(g)).Position);
                pop(bestcost_bound(worstcost_2).index(g)).bound = bestcost_bound(worstcost_4).bound;
            end
        ...
        if  (u>3) && (u<=14)
            ...
            cood.cost = [];           
            move = repmat(cood, 1, 1);
            targetrole = 'roaming';      
            matches = find(cellfun(@(b) isequal(b, targetrole), {pop.role}));
           for i =1 :length(matches)
               pos = pop(matches(i)).Position;
               previous_cost = pop(matches(i)).Cost;
               min_pos = min(pos)*ones(1,nVar);
               max_pos = max(pos)*ones(1,nVar);
               delta1 = max_pos - pos;
               delta2 = min_pos -pos;
               new_pos1 = pos + delta1*rand();
               new_pos2 = pos + delta2*rand();
               cost1 = CostFunction(new_pos1);
               cost2 = CostFunction(new_pos2);
               if cost1<cost2 && cost1<previous_cost
                   pop(matches(i)).Position = new_pos1;
                   pop(matches(i)).Cost = cost1;
               elseif cost1>cost2 && cost2<previous_cost
                   pop(matches(i)).Position = new_pos2;
                   pop(matches(i)).Cost = cost2;
               end             
           end
           costs = [pop(matches).Cost];
           [~,index] = min(costs);
           gbest_pos = pop(matches(index)).Position;
           bound = pop(matches(index)).bound;
           cost_best = pop(matches(index)).Cost;


           %เคลื่อนที่หา roaming mี่ดีสุด
           for r = 1:length(matches)
               pos = pop(matches(r)).Position;
               if pos == gbest_pos
                   bmax = bound(1,2);
                   bmin = bound(1,1);
                   result1 = bmax * ones(1, nVar);
                   result2 = bmin * ones(1, nVar);

                   delta1 = result1-pos;
                   delta2 = result2-pos;

                   pos1 = pos + delta1*rand();
                   pos2 = pos + delta2*rand();

                   cost1 = CostFunction(pos1);
                   cost2 = CostFunction(pos2);

                   if cost1<cost2 && cost1<cost_best
                        pop(matches(r)).Position = pos1;
                        pop(matches(r)).Cost =cost1;


                   elseif cost2<cost1 && cost2<cost_best
                        pop(matches(r)).Position = pos2;
                        pop(matches(r)).Cost =cost2;
                   ...
                   delta = gbest_pos-pos;
                   new_pos = pos+delta*(rand());
                   cost = CostFunction(new_pos);
                   pop(matches(r)).Position = new_pos;
                   pop(matches(r)).Cost =cost;
               end
           ...
           matches = find(cellfun(@(b) isequal(b, targetrole), {pop.role}));
           cost = [];
           for e =1:length(matches)               
               cost(end+1) = pop(matches(e)).Cost;
           end
           
           [~,index_frontline] = min(cost);
           Gr_bestpos = pop(index_frontline).Position;
           w = wmax - (wmax-wmin)*u/MaxIt;
           for e =1:length(matches)               
               pos = pop(matches(e)).Position;
               pop(matches(e)).Velocity = w*pop(matches(e)).Velocity + c1*rand(1,nVar).*(Gr_bestpos-pos) + c2*rand(1,nVar).*(GlobalBest.Position-pos);
               % Apply Velocity Limits
                pop(matches(e)).Velocity = max(pop(matches(e)).Velocity, VelMin);
                pop(matches(e)).Velocity = min(pop(matches(e)).Velocity, VelMax);

               pos_new = pos + pop(matches(e)).Velocity;
               cost_new = CostFunction(pos_new);
               cost_previous = pop(matches(e)).Cost;

               if cost_new<cost_previous
                   pop(matches(e)).Position = pos_new;
                   pop(matches(e)).Cost = cost_new;
                   pop(matches(e)).Cost
...
function isInRange = checkInRange(value,index, bound)
    
    if isequal(size(bound{index}), [2, 2])
        isInRange = ((value >= bound{index}(1,1)) & (value <= bound{index}(1,2))) | ((value >= bound{index}(2,1)) & (value <= bound{index}(2,2)));
    else
        size_current_bounds = size(bound{index});
        col_bound = size_current_bounds(2);
        for i=1:(col_bound/2)
             isInRange = ((value >= bound{index}(1,i)) & (value <= bound{index}(1,i+(col_bound/2)))) | ((value >= bound{index}(2,i)) & (value <= bound{index}(2,i+(col_bound/2))));
        end
