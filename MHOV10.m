...
% F = 'F16'; % Change this to any function ID from 'F1' to 'F23'
F = 'ThreeBarTruss';
...
function [BestSol, BestCost, Convergence] = MHO_Adaptive(problem, params)
    ...
    Hunter = struct('role', [], 'Best_Position', [], 'Best_DMG', [],'fitness_value',[] , 'Try_move',[],'DMG',[]);
    ...
    for i = 1:nPop        
        ...
        if Hunter(i).Best_DMG < 0
            Hunter(i).fitness_value = 1+ abs(Hunter(i).Best_DMG);
        else
            Hunter(i).fitness_value = 1/(1+Hunter(i).Best_DMG);
        end
    ...
    for u = 1:MaxIt
        ...
        for i = matches_BladeMaster
           ...
           for j = 1:nVar
                ...
                new_dim_plus = dim + (2+(-(u/MaxIt)*rand()))*(Hunter1st_rank.Best_Position(j)-dim) ;  
...
%                 new_dim_plus = max(VarMin(j), min(VarMax(j), new_dim_plus));                   
            ...
            Hunter(i).Try_move = Test_Delta1;                
            Hunter(i).DMG = CostFunction(Hunter(i).Try_move);
                
        
            
            if Hunter(i).DMG < Hunter(i).Best_DMG
                Hunter(i).Best_DMG = Hunter(i).DMG;
                Hunter(i).Best_Position = Hunter(i).Try_move;                                
            end
       ...
       for i = matches_frontline            
           ...
           for j = 1:nVar
               ...
               new_dim_plus = dim + phi * ((1 - u/MaxIt) * abs(Hunter1st_rank.Best_Position(j)-dim));
%                 new_dim_plus = dim + ((1 - u/MaxIt) * rand())*(Hunter1st_rank.Best_Position(j)-dim);  
                if new_dim_plus < VarMin(j)
                    new_dim_plus = 2 * VarMin(j) - new_dim_plus; 
                elseif new_dim_plus > VarMax(j)
                    new_dim_plus = 2 * VarMax(j) - new_dim_plus;
...
%                 new_dim_plus = max(VarMin(j), min(VarMax(j), new_dim_plus));
         ...
         for i = matches_Ranger
             ...
             if safety_sum > stun_gauge %บอสติดสตั้น เลยเข้ามาตีใกล้ๆ
                 ...
                 Hunter(i).Try_move = new_pos_plus;                     
             ...
             elseif safety_sum < stun_gauge %บอสโจมตีไปมา ต้องทิ้งระยะเพื่อความปลอดภัย
                 ...
                 Hunter(i).Try_move = max(VarMin, min(VarMax, pos + (-1+2*rand())*(new_pos_plus-pos)));                 
                 
                     
             end          
             if CostFunction(Hunter(i).Try_move)<Hunter(i).Best_DMG 
                Hunter(i).Best_Position = Hunter(i).Try_move;
                Hunter(i).Best_DMG = CostFunction(Hunter(i).Best_Position);
             end
       ...
       for i = matches_Supporter
           ...
           Try_move = Hunter(i).Try_move;            
           
           
           if rand()<0.5
               
               new_pos = Hunter(i).Try_move;
               for j = 1:nVar              
                   if rand()<0.7
               ...
               if CostFunction(new_pos) < Hunter(i).Best_DMG
                   Hunter(i).Best_Position = new_pos;
                   Hunter(i).Best_DMG = CostFunction(new_pos);
               end
               use_pos = Try_move + rand()*(new_pos-Try_move);
               Hunter(i).Try_move = use_pos;
               Hunter(i).DMG = CostFunction(use_pos);
               if Hunter(i).DMG<Hunter(i).Best_DMG
                   Hunter(i).Best_Position = Hunter(i).Try_move;
                   Hunter(i).Best_DMG =  Hunter(i).DMG ;
               end               
